...
from .genericfs import GenericFSError, NoSuchFile
...
class GenericContentsManager(ContentsManager, HasTraits):
    ...
    def file_exists(self, path):
        ...
        self.log.debug("coscontents.GenericManager.file_exists: ('%s')", path)
        return self.fs.isfile(path)

    def dir_exists(self, path):
        # Does a directory exist at the given path?
        self.log.debug(
            "coscontents.GenericManager.dir_exists: path('%s')", path
        )
    ...
    def get(self, path, content=True, type=None, format=None):
        ...
        self.log.debug(
            "coscontents.GenericManager.get: path('%s') type(%s) format(%s)",
            path,
            type,
            format,
        )
        ...
        if self.parent:
            if path.startswith(self.parent.root_dir):
                self.log.debug(
                    "coscontents.GenericManager.get: removing root_dir (%s) from path",
    ...
    def _get_directory(self, path, content=True, format=None):
        self.log.debug(
            "coscontents.GenericManager._get_directory: path('%s') content(%s) format(%s)",
            path,
            content,
            format,
        )
    ...
    def _get_notebook(self, path, content=True, format=None):
        self.log.debug(
            "coscontents.GenericManager._get_notebook: path('%s') type(%s) format(%s)",
            path,
            content,
            format,
        )
    ...
    def _get_file(self, path, content=True, format=None):
        self.log.debug(
            "coscontents.GenericManager._get_file: path('%s') type(%s) format(%s)",
            path,
            content,
            format,
        )
    ...
    def _directory_model_from_path(self, path, content=False):
        self.log.debug(
            "coscontents.GenericManager._directory_model_from_path: path('%s') type(%s)",
            path,
            content,
        )
    ...
    def save(self, model, path):
        ...
        self.log.debug("coscontents.GenericManager.save %s: '%s'", model, path)
    ...
    def _save_large_file(self, chunk, model, path, format):
        ...
        self.log.debug(
            "coscontents.GenericManager.save (chunk %s) %s: '%s'",
            chunk,
            model,
            path,
        )
        ...
        try:
            ...
            self.log.error(
                "coscontents.GenericManager._save_large_file: error while saving file: %s %s",
                path,
                e,
                exc_info=True,
            )
    ...
    def rename_file(self, old_path, new_path):
        ...
        """
        self.log.debug(
            "coscontents.GenericManager.rename_file: Init rename of '%s' to '%s'",
            old_path,
            new_path,
        )
        if self.file_exists(new_path) or self.dir_exists(new_path):
            ...
            self.log.debug(
                "coscontents.GenericManager: Actually renaming '%s' to '%s'",
                old_path,
                new_path,
            )
    ...
    def delete_file(self, path):
        ...
        self.log.debug("coscontents.GenericManager.delete_file '%s'", path)
    ...
    def is_hidden(self, path):
        ...
        self.log.debug("coscontents.GenericManager.is_hidden '%s'", path)
        return False
