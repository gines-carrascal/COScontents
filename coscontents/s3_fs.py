"""
Utilities to make IBM COS look like a regular file system
"""
...
class S3FS(GenericFS):
    ...
    def ls(self, path=""):
        ...
        self.log.debug("coscontents.S3FS.ls: Listing directory: `%s`", path_)
    ...
    def isfile(self, path):
        ...
        self.log.debug("coscontents.S3FS: `%s` is a file: %s", path_, is_file)
        return is_file
    ...
    def isdir(self, path):
        ...
        self.log.debug(
            "coscontents.S3FS: `%s` is a directory: %s", path_, is_dir
        )
        return is_dir

    def mv(self, old_path, new_path):
        self.log.debug(
            "coscontents.S3FS: Move file `%s` to `%s`", old_path, new_path
        )
    ...
    def cp(self, old_path, new_path):
        ...
        self.log.debug(
            "coscontents.S3FS: Copying `%s` to `%s`", old_path_, new_path_
        )
    ...
    def rm(self, path):
        path_ = self.path(path)
        self.log.debug("coscontents.S3FS: Removing: `%s`", path_)
        if self.isfile(path):
            self.log.debug("coscontents.S3FS: Removing file: `%s`", path_)
            self.fs.rm(path_)
        elif self.isdir(path):
            self.log.debug("coscontents.S3FS: Removing directory: `%s`", path_)
    ...
    def mkdir(self, path):
        ...
        self.log.debug("coscontents.S3FS: Making dir: `%s`", path_)
        self.fs.touch(path_)
    ...
    def write(self, path, content, format):
        ...
        self.log.debug("coscontents.S3FS: Writing file: `%s`", path_)
    ...
    def writenotebook(self, path, content):
        path_ = self.path(self.remove_prefix(path))
        self.log.debug("coscontents.S3FS: Writing notebook: `%s`", path_)
        with self.fs.open(path_, mode="wb") as f:
