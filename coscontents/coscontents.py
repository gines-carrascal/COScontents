...
from .genericmanager import GenericContentsManager, from_dict
from .s3_fs import S3FS


class COSContentsManager(GenericContentsManager):
...
def validate_bucket(user_bucket, log):
    ...
    log.debug(
        f"cosmanager.validate_bucket: User provided bucket: {user_bucket}"
    )
